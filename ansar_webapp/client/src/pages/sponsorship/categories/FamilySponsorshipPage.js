...
import { FaHandHoldingHeart, FaUser, FaGraduationCap, FaTools, FaUtensils, FaHeart, FaHome } from 'react-icons/fa';
...
const FamilySponsorshipPage = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Family Sponsorship | Ansar Organization";
    window.scrollTo(0, 0);
    fetch('/api/sponsorship-category/family')
      .then(res => res.json())
      .then(json => {
        setData(json);
        setIsLoading(false);
  ...
  const filteredFamilies = Array.isArray(items)
    ? (activeFilter === 'all'
        ? items
        : items.filter(family => family.type === activeFilter))
    : [];
  return (
    <div>
      {/* Hero Section */}
          ...
          <div className="max-w-3xl">
            ...
            >
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
                <span className="text-primary-300">{hero?.titleHighlight || 'Family'}</span> {hero?.titleRest || 'Sponsorship'}
              </h1>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          {filteredFamilies.length === 0 ? (
            <div className="text-center text-gray-400 py-16">No families found for this filter.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredFamilies.map((family) => (
                ...
                >
                  <div className="h-48 bg-cover bg-center" style={{ backgroundImage: `url('${family.image || ''}')` }}></div>
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-xl font-bold text-gray-800">{family.name}</h3>
                      <span className="text-sm text-gray-500">{family.members} members</span>
                    </div>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-sm text-gray-500">{family.location}</span>
                      {family.sponsored && (
                    ...
                    <div className="space-y-4">
                      <div className="flex flex-wrap gap-2">
                        ...
                        >
                          {family.type.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                        </span>
                      </div>
            ...
            {Array.isArray(benefits?.items) && benefits.items.length > 0 ? benefits.items.map((benefit, idx) => (
              ...
              >
                <div className="bg-primary-100 p-4 w-16 h-16 rounded-2xl mb-4 flex items-center justify-center text-primary-600">
                  {benefit.icon === 'food' && <FaUtensils size={24} />}
                  {benefit.icon === 'construction' && <FaTools size={24} />}

...
import { FaHandHoldingHeart, FaUser, FaGraduationCap, FaHome, FaHeart } from 'react-icons/fa';
import { useDonation } from '../../../contexts/DonationContext';


const OrphanSponsorshipPage = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Orphan Sponsorship | Ansar Organization";
    window.scrollTo(0, 0);
    fetch('/api/sponsorship-category/orphan')
      .then(res => res.json())
      .then(json => {
        setData(json);
        setIsLoading(false);
  ...
  if (isLoading) return <div className="flex justify-center items-center h-64"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-primary-600"></div></div>;
  if (!data) return <div className="text-center py-16">No data found.</div>;

  const { hero, filters, items, impactStats, benefits } = data;
  const filteredOrphans = Array.isArray(items)
    ? (activeFilter === 'all'
        ? items
        : items.filter(orphan => Array.isArray(orphan.needs) && orphan.needs.includes(activeFilter)))
    : [];
  return (
    <div>
      {/* Hero Section */}
          ...
          <div className="max-w-3xl">
            ...
            >
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
                <span className="text-primary-300">{hero?.titleHighlight || 'Orphan'}</span> {hero?.titleRest || 'Sponsorship'}
              </h1>
            ...
            {Array.isArray(impactStats) && impactStats.length > 0 ? impactStats.map((stat, idx) => (
              ...
              >
                <div className="bg-primary-100 p-4 w-16 h-16 rounded-2xl mb-4 flex items-center justify-center text-primary-600 mx-auto">
                  {stat.icon === 'user' && <FaUser size={24} />}
                  {stat.icon === 'graduation' && <FaGraduationCap size={24} />}
                  {stat.icon === 'home' && <FaHome size={24} />}
                  {stat.icon === 'heart' && <FaHeart size={24} />}
                </div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          {filteredOrphans.length === 0 ? (
            <div className="text-center text-gray-400 py-16">No orphans found for this filter.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredOrphans.map((orphan) => (
                ...
                >
                  <div className="h-48 bg-cover bg-center" style={{ backgroundImage: `url('${orphan.image || ''}')` }}></div>
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-xl font-bold text-gray-800">{orphan.name}</h3>
                      <span className="text-sm text-gray-500">{orphan.age} years</span>
                    </div>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-sm text-gray-500">{orphan.location}</span>
                      {orphan.sponsored && (
                    ...
                    <p className="text-gray-600 mb-4">{orphan.story}</p>
                    <div className="space-y-4">
                      <div className="flex flex-wrap gap-2">
                        {Array.isArray(orphan.needs) && orphan.needs.length > 0 ? orphan.needs.map((need) => (
                          <span
                            key={need}
                        ...
                        )) : <span className="text-gray-400">No needs listed.</span>}
                      </div>
            ...
            {Array.isArray(benefits?.items) && benefits.items.length > 0 ? benefits.items.map((benefit, idx) => (
              ...
              >
                <div className="bg-primary-100 p-4 w-16 h-16 rounded-2xl mb-4 flex items-center justify-center text-primary-600">
                  {benefit.icon === 'graduation' && <FaGraduationCap size={24} />}
                  {benefit.icon === 'heart' && <FaHeart size={24} />}
                  {benefit.icon === 'home' && <FaHome size={24} />}
                </div>
...
export default OrphanSponsorshipPage;
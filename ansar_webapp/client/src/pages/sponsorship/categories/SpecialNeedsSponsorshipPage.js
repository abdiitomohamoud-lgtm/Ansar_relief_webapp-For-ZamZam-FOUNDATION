...
import { FaHandHoldingHeart, FaWheelchair, FaHandsHelping, FaGraduationCap, FaUserFriends, FaHospital } from 'react-icons/fa';
import { useDonation } from '../../../contexts/DonationContext';

const SpecialNeedsSponsorshipPage = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Special Needs Sponsorship | Ansar Organization";
    window.scrollTo(0, 0);
    fetch('/api/sponsorship-category/special-needs')
      .then(res => res.json())
      .then(json => {
        setData(json);
        setIsLoading(false);
  ...
  const filteredIndividuals = activeFilter === 'all'
    ? items
    : items.filter(individual => individual.type === activeFilter);
  return (
    <>
      {/* Hero Section */}
      <section
          ...
          <div className="max-w-3xl">
            ...
            >
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
                <span className="text-primary-300">{hero?.titleHighlight || 'Special Needs'}</span> {hero?.titleRest || 'Sponsorship'}
              </h1>
            ...
            {Array.isArray(impactStats) && impactStats.length > 0 ? impactStats.map((stat, idx) => (
              ...
              >
                <div className="bg-primary-100 p-4 w-16 h-16 rounded-2xl mb-4 flex items-center justify-center text-primary-600 mx-auto">
                  {stat.icon === 'wheelchair' && <FaWheelchair size={24} />}
                  {stat.icon === 'hospital' && <FaHospital size={24} />}
                  {stat.icon === 'user-friends' && <FaUserFriends size={24} />}
                </div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          {filteredIndividuals.length === 0 ? (
            <div className="text-center text-gray-400 py-16">No individuals found for this filter.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredIndividuals.map((individual) => (
                ...
                >
                  <div className="h-48 bg-cover bg-center" style={{ backgroundImage: `url('${individual.image || ''}')` }}></div>
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-xl font-bold text-gray-800">{individual.name}</h3>
                      <span className="text-sm text-gray-500">{individual.type.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</span>
                    </div>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-sm text-gray-500">{individual.location}</span>
                    ...
                    <p className="text-gray-600 mb-4">{individual.story}</p>
                    <div className="space-y-4">
                      <div className="flex flex-wrap gap-2">
                        <span className="px-2 py-1 bg-primary-100 text-primary-600 rounded-full text-xs font-medium">
                          {individual.type.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Monthly Cost</span>
                        <span className="text-lg font-bold text-primary-600">
                          ${individual.monthlyCost}
                        </span>
                      </div>
                      {individual.sponsored ? (
                        <button
            ...
            {Array.isArray(benefits?.items) && benefits.items.length > 0 ? benefits.items.map((benefit, idx) => (
              ...
              >
                <div className="bg-primary-100 p-4 w-16 h-16 rounded-2xl mb-4 flex items-center justify-center text-primary-600">
                  {benefit.icon === 'hands-helping' && <FaHandsHelping size={24} />}
                  {benefit.icon === 'wheelchair' && <FaWheelchair size={24} />}
...
export default SpecialNeedsSponsorshipPage;
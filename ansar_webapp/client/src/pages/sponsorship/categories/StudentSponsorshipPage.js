...
const StudentSponsorshipPage = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Student Sponsorship | Ansar Organization";
    window.scrollTo(0, 0);
    fetch('/api/sponsorship-category/student')
      .then(res => res.json())
      .then(json => {
        setData(json);
        setIsLoading(false);
  ...
  const filteredStudents = Array.isArray(items)
    ? (activeFilter === 'all'
        ? items
        : items.filter(student => student.program === activeFilter))
    : [];
  return (
    <div>
      {/* Hero Section */}
          ...
          <div className="max-w-3xl">
            ...
            >
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
                <span className="text-primary-300">{hero?.titleHighlight || 'Student'}</span> {hero?.titleRest || 'Sponsorship'}
              </h1>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          {filteredStudents.length === 0 ? (
            <div className="text-center text-gray-400 py-16">No students found for this filter.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredStudents.map((student) => (
                ...
                >
                  <div className="h-48 bg-cover bg-center" style={{ backgroundImage: `url('${student.image || ''}')` }}></div>
                  <div className="p-6">
                    ...
                    <p className="text-gray-600 mb-4">{student.story}</p>
                    <div className="space-y-4">
                      <div className="flex flex-wrap gap-2">
                        <span
                          className="px-2 py-1 bg-primary-100 text-primary-600 rounded-full text-xs font-medium"
                        >
                          {student.program.charAt(0).toUpperCase() + student.program.slice(1)}
                        </span>
                      </div>
